...
source $(dirname "$0")/operaton-lib.sh
...
DB_URL=${DB_URL:-jdbc:h2:./operaton-h2-dbs/process-engine;TRACE_LEVEL_FILE=0;DB_CLOSE_ON_EXIT=FALSE}
...
EOF
  ...
  rm -rf /operaton/standalone/configuration/standalone_xml_history/current/* batch.cli
}
...
CMD="/operaton/bin/standalone.sh"
...
if [ "$JMX_PROMETHEUS" = "true" ] ; then
  ...
  LOG_MANAGER_PATH=$(find /operaton/modules -name "jboss-logmanager*.jar")
  COMMON_PATH=$(find /operaton/modules -name "wildfly-common*.jar")
  ...
  export PREPEND_JAVA_OPTS="${PREPEND_JAVA_OPTS} -javaagent:/operaton/javaagent/jmx_prometheus_javaagent.jar=${JMX_PROMETHEUS_PORT}:${JMX_PROMETHEUS_CONF}"
fi
